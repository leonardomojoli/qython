// Pílulas do copiloto (mobile) — paridade com o web. v1 fixo no app; v2 (runtime) vem da API
// GET /api/copilot/suggested-prompts (curar sem deploy). Esta lista é o fallback offline.

export interface CopilotPill {
  id: string; // slug estável
  category: string;
  icon: string;
  labelKey: string; // chave i18n (cai no `label` se faltar)
  label: string; // fallback pt
  opener: string; // texto injetado no input
}

export const COPILOT_PROMPTS: CopilotPill[] = [
  {
    id: 'study-schedule',
    category: 'academico',
    icon: '📅',
...
// Amostra N pílulas aleatórias (o conteúdo "rotaciona", não a posição).
export function sampleFrom<T>(arr: T[], n: number): T[] {
